...
const createCanvasWithCenteredText = async (
  ...
  ctx.font = `${
    property.fontStyle === "italic" ? "italic" : "normal"
...
router.post(
  ...
  async (req, res) => {
    try {
      console.log("vvvvvvvvvvvvvvvvvvvv 2", req.files)
      ...
      const csvFilePath =
        isSample === "true" ? "" : req.files.guestNames[0].path;
      const inputPath = req.files[0].originalname;
